...
def visualize(img1, faces1, img2, faces2, matches, scores, target_size=[512, 512], fps=None): # target_size: (h, w)
    out1 = img1.copy()
    out2 = img2.copy()
    ...
    # if fps is not None:
    #     cv.putText(padded_out2, 'FPS: {:.2f}'.format(fps), (0, 15), cv.FONT_HERSHEY_SIMPLEX, 0.5, text_color)
...
if __name__ == '__main__':
    ...
    if args.query is not None:
        ...
        while cv.waitKey(1) < 0:
            ...
            # Inference
            tm.start()
            faces2 = detector.infer(img2) # results is a tuple
            tm.stop()
            ...
            # Draw results
            image = visualize(img1, faces1, img2, faces2, matches, scores, fps=tm.getFPS())
